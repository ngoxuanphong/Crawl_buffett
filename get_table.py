...
bs_text = ["四半期連結貸借対照表", "連結貸借対照表", "連結財政状態計算書"]
...
def extract_data_pdf_bs(
    ...
    if find_key == True:
        ...
        # Tìm số cột
        COUNT_ = 10
        for str in table_text.split("\n"):  # find count columns by 10 first rows
            COUNT_ -= 1
            row_ = str.split(" ")
            row = []
            for s in row_:
...
def get_table(id_company, type_="bs", path_save="", return_df=True, save_file=False):
    for file in os.listdir(path_save + f"Data/{id_company}/PDF"):
        if "ocr" not in file and "(訂正)" not in file:
            try:
                ...
                if save_file:
                    df_.to_csv(
                        path_save + f"Data/{id_company}/docs/{type_}/{year}_{quy}.csv",
                        index=False,
                    )
                if return_df:
